...
public class Rook extends Piece {
	...
	private static final int[][] move_list = {{0,1},{0,-1},{-1,0},{1,0}};
		...
		for(int i = 0;i < sizemove;i++) { 
			...
			while(true) {
				...
				if(info == PieceInfo.NONE) {
					if(!onlyAttack) move.add(new Move("R",toPos(), toPos(newx,newy)));
					newx += move_list[i][0];
					newy += move_list[i][1];
				} else {
				if(!isSameColor(info,whiteturn)) {
					move.add(new AttackMove("R",toPos(), toPos(newx,newy)));
				}
					break;	
				}
			}
